			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="car.motor.label" default="Motor" /></th>
					
						<g:sortableColumn property="name" title="${message(code: 'car.name.label', default: 'Name')}" />
					
						<th><g:message code="car.person.label" default="Person" /></th>
				...
				<g:each in="${carInstanceList}" status="i" var="carInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${carInstance.id}">${fieldValue(bean: carInstance, field: "motor")}</g:link></td>
					
						<td>${fieldValue(bean: carInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: carInstance, field: "person")}</td>
